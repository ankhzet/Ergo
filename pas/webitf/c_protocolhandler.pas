...
procedure NewProtocolHandler(const aProtocolName: WideString; aProtocolCallback: TProtocolCallback; aProtocollType: TGUID);
procedure NewHttpProtocolHandler(const aProtocolName: WideString; aProtocolCallback: TProtocolCallback);
...
begin
	...
	InternetSession.RegisterNameSpace(Factory, Class_AsyncPlugProto_Protocol, PWideChar(_protocol), 0, nil, 0);
...
begin
	if Assigned(InternetSession) then
		InternetSession.UnregisterNameSpace(Factory, PWideChar(_protocol));
...
begin
	...
	FURL := Copy(aURL, Pos(':', aURL) + 1, MaxInt);
	Result := True;
end;
...
begin
	...
	FProtSink.ReportProgress(BINDSTATUS_VERIFIEDMIMETYPEAVAILABLE,  PWideChar(mimeType));
