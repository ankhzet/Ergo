...
uses
	strings, functions, Windows, Classes, Graphics, JPEG,
...
type
  ...
  public
    ...
    class function CanLoad(const FileName: AnsiString): Boolean; overload; virtual;
  ...
  TFilterOption = (
    foCompact,          // use the compact form in filter AnsiStrings instead listing each extension on a separate line
  ...
  // graphic formats nor to build common entries in filter AnsiStrings nor does it care for duplicate entries or
...
procedure GraphicExError(ErrorAnsiString: AnsiString); overload;

begin
  raise EInvalidGraphic.Create(ErrorAnsiString);
end;

//----------------------------------------------------------------------------------------------------------------------

procedure GraphicExError(ErrorAnsiString: AnsiString; Args: array of const); overload;

begin
  raise EInvalidGraphic.CreateFmt(ErrorAnsiString, Args);
end;
...
begin
	result := Byte((Value > -0.5) and (Value <= 0.5));
end;
...
begin
  if Value < 0 then Value := -Value;
	if Value < 3 then begin
		...
		begin
			Value := Value * Pi;
			Result := (Sin(Value) * Sin(Value / 3)) / (Value * Value / 3);
		end
		else Result := 1;
	end
							 else Result := 0;
  ...
  try
    ...
    begin
      ...
      begin
        ContributorList[I].N := 0;
				...
				Center := I / ScaleX;
				Left := trunc(Center - Width);
				Right := round(Center + Width + 0.5);
    ...
    begin
      ...
      begin
        ...
        Center := I / ScaleX;
        Left := trunc(Center - Radius);
				Right := round(Center + Radius + 0.5);
        for J := Left to Right do
        begin
...
class function TGraphicExGraphic.CanLoad(const FileName: AnsiString): Boolean;
...
procedure TCUTGraphic.LoadFromFile(const FileName: AnsiString);
...
begin
  ...
  begin
    ...
    begin
			Version := sti(Copy(Header.Version, 1, 2));
      ColorScheme := csIndexed;
      SamplesPerPixel := 1;
...
begin
	...
	begin
		...
		begin
			...
			Result := (PCardinal(@Magic[0])^ = PCardinal(@PNGMagic[0])^)
				and (PCardinal(@Magic[4])^ = PCardinal(@PNGMagic[4])^);// CompareMem(@Magic, @PNGMagic, 8);
		end;
		Position := LastPosition;
	end;
end;
  ...
  case Filter of
    ...
    3: // average filter
      begin
        ...
        for I := 0 to BPP - 1 do
        begin
          TargetRun^ := Byte(Raw^ + trunc(Prior^ / 2));
          Inc(Raw);
          Inc(Prior);
          Inc(TargetRun);
        end;
        ...
        while BytesPerRow > 0 do
        begin
					TargetRun^ := Byte(Raw^ + trunc((Decoded^ + Prior^) / 2));
          Inc(Raw);
          Inc(Decoded);
          Inc(Prior);
          Inc(TargetRun);
...
begin
  ...
  begin
    ReadBuffer(Magic, 8);
    if (PCardinal(@Magic[0])^ = PCardinal(@PNGMagic[0])^)
				and (PCardinal(@Magic[4])^ = PCardinal(@PNGMagic[4])^) then
    begin
      // first chunk must be an IHDR chunk
      FCurrentCRC := LoadAndSwapHeader;
...
begin
  ...
  try
    ...
    BytesPerRow := TargetBPP * ((Width * TIHDRChunk(Description).BitDepth + 7) div 8) + 1;
		...
		RowBuffer[True] := GetMemory(BytesPerRow);
		RowBuffer[False] := GetMemory(BytesPerRow);
  ...
  begin
    case ColorType of
      ...
      // read the values (at most 256)...
			Move(FRawBuffer^,  FTransparency[0], iMax(FHeader.Length, 256));
...
begin
  ...
  begin
    ...
    Dispose(PClassEntry(FClassList[I])); // need Dispose with type casting to free AnsiStrings too
  end;
  FClassList.Clear;
...
begin
  ...
  if Extension <> '' then
    for I := 0 to FExtensionList.Count - 1 do
			if lstrcmpi(@PExtensionEntry(FExtensionList[I]).Extension[1], @Extension[1]) = 0 then
      begin
        Result := I;
        Break;
      end;
...
begin
    ...
    begin
      ...
      begin
        ...
        SetLength(S, Length(S) - 2);
				if S <> '' then DL.AddObject(ClassEntry.Description, Pointer(PChar(AnsiString(PAnsiChar(S)))));
      end;
    end;
  end
  else
    ...
    begin
      ...
      begin
        ...
        if S = '' then S := ExtEntry.ClassReference.Description;
				DL.AddObject(S, Pointer(PChar(AnsIString(PAnsiChar('*.' + ExtEntry.Extension)))));
        if foIncludeAll in Options then All.Add(ExtEntry.Extension);
      end;
    end;
	end;

  // build final filter AnsiString out of the collected sub AnsiStrings
  ...
  begin
		...
		freemem(PAnsiChar(DL.Objects[I]));
...
//       uniquely identifyable (e.g. Dr. Halo *.cut images).
	...
	function CanLoad(GClass: TGraphicClass): Boolean;
	begin
		result := true;
		with GClass.Create do
			try
				try
...
begin
	...
	for I := 0 to FClassList.Count - 1 do begin
		...
		if T.InheritsFrom(TGraphicExGraphic) then
			if T.CanLoad(Stream) then begin
				Result := T;
				Break;
			end else
		else
			if CanLoad(T) then begin
				Result := T;
				Break;
			end;
	end;
